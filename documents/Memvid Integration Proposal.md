...
> **Supersedes:** `MEMVID_PROPOSAL.md` and `Memvid Integration Proposal v2.md`
> **Contributors:** Gemini, Codex, Claude Opus 4.6
> **Date:** 2026-02-09
...
Embedding generation should run server-side (Modal), not on device.

---

## Decision Requested

- [ ] Approve Phase 0 now (identity injection + 1-week evidence capture)
- [ ] Approve Phase 1 only if evidence shows repeated memory misses
- [ ] Defer Memvid beyond Phase 1 if KV summaries are sufficient
- [ ] Timebox Phase 2 spike before committing to full memory platform work

---

## Historical Council Review Summary

Reviewed in earlier draft cycle by Gemini and Codex.

Key carry-forward conclusions:

- Defer codebase semantic indexing or treat it as a separate pipeline
- Keep strict namespacing and provenance on memory entries
- Preserve user control over memory persistence (pin/forget)
- Treat prompt-injection persistence as a first-class risk
